"""
발화(STT 텍스트)가 DB에서 가져온 image관련 json 요소들과 얼마나 일치하는지 판단.
...
def load_concepts_from_db(image_id: str) -> dict:
    """실제 서비스용 — Oracle DB에서 concepts JSON 조회."""
    load_dotenv()
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT concepts_json FROM images WHERE image_id = :image_id",
                image_id=image_id,
            )
            row = cursor.fetchone()
            if row is None:
                raise ValueError(f"image_id={image_id}에 해당하는 데이터가 없습니다.")

            concepts_json = row[0]
            if hasattr(concepts_json, "read"):
                concepts_json = concepts_json.read()
...
#     transcript = "어 여자는 벚꽃 음 속에서 자 음 걷고 있어요"
